...
class Manager():
    def __init__(self, train_loader, eval_loader, model, learning_rate, save_model_path):
        ...
        self.summarywriter_eval = SummaryWriter(comment='-baseline-eval')
    ...
    def fit(self):
        ...
        for epoch in range(2000):
            for step, (mel, labels) in enumerate(self.train_loader):
                mel = mel.permute(0, 2, 1)
                mel_shape = mel.shape
                mel = mel.reshape(-1, 1, int(mel_shape[-1]/10), mel_shape[-1])
                mel  = mel.to(torch.float).cuda()
                labels = labels.cuda()
                out = self.model(mel)
                ...
                if step % 3 ==0:
                    ...
                    for r, p, f, diagnosis in zip(recall, precision, F1_score, self.disease_index.keys()):
                        self.summarywriter_train.add_scalars('indicators', {'F1_score_'+diagnosis:f.item(),
                                                              ...
                                                              'Precision_'+diagnosis:p.item()}, total_step)
                    self.summarywriter_train.add_scalar('loss', loss.item(), total_step)
    ...
    def eval(self, epoch):
        ...
        label_list = []
        with torch.no_grad():
            for i, (mel, labels) in enumerate(self.eval_loader):
                raw_shape = mel.shape
                mel = mel.reshape(-1, 1, raw_shape[-2], raw_shape[-1])
                mel = mel.permute(0, 1, 3, 2)
                mel = mel.to(torch.float).cuda()
                out.append(self.model(mel))
                label_list.append(labels.cuda())
